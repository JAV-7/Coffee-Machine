...
from coffee_machine import (
    ...
    make_coffee, is_empty, is_full, MENU
)
from machine_operations import withdraw_money, deposit_money
...
def random_test_case_1_view_status():
    """Random Test Case 1: View machine status with random initial resources"""
    ...
    print(f"Random initial state generated:")
    _, output = capture_output(show_data, machine)
    print(output)
    return machine

def random_test_case_2_fill_machine():
    ...
    print(f"Random inputs: {beans_to_add}g beans, {water_to_add}ml water, {milk_to_add}ml milk, {cups_to_add} cups")
    
    with patch('builtins.input', side_effect=[str(beans_to_add), str(water_to_add), str(milk_to_add), str(cups_to_add)]):
...
def random_test_case_3_successful_purchase():
...
def random_test_case_4_insufficient_payment():
    ...
    with patch('builtins.input', side_effect=[selected_coffee, str(insufficient_payment)]):
        _, output = capture_output(make_coffee, machine)
...
def random_test_case_5_admin_operations():
    ...
    with patch('builtins.input', side_effect=[str(deposit_amount)]):
        _, output1 = capture_output(deposit_money, machine)
    ...
    with patch('builtins.input', side_effect=[str(max_withdraw), charity_choice]):
...
def random_test_case_6_resource_limits():
    """Random Test Case 6: Test resource limits and edge cases"""
    print("=== RANDOM TEST CASE 6: Random Resource Limits Test ===")
    machine = create_coffee_machine()
    
    # Create a scenario with random low resources
    machine['beans'] = random.randint(0, 30)  # Possibly not enough for some coffees
